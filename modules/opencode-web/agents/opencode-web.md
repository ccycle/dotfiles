...
zotコマンドでZoteroライブラリのメタデータ・添付PDFの全文・ノート本文を検索します。qmodeは必ずeverythingを指定してください（デフォルトのtitleCreatorYearだとタイトル等しか検索されません）。

```
zot items list --query "検索語" --qmode everything
```

### 曖昧な想起への対応

ユーザーは検索語を正確に思い出せないことが多くあります。一度の検索で見つからなくても諦めず、次を試してください。

- 類義語・関連語で、日本語と英語の両方を使って複数回検索し直す
- `rg --files .` でファイル名一覧、`zot items list --qmode everything` で候補タイトル一覧を先に眺めて、内容から近そうなものに見当をつける
- 見当をつけたノート・アイテムは内容を読んで確認する
