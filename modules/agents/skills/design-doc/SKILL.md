...
Maintain one `design.md` per module or feature, placed in the same directory as the code it describes. It is a living document describing the *current* design intent — not an append-only decision log (ADR). When a decision changes, rewrite the affected section in the same commit as the code change.
