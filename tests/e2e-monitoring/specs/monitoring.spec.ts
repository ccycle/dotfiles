...
test('Prometheus loads the real scrape config and discovers every configured job', async ({ request }) => {
  ...
  for (const job of expectedJobs) {
    expect(discoveredJobs.has(job), `job "${job}" (from prometheus.yml) has a discovered target`).toBe(true);
  }
  ...
  console.log(
    ...
    JSON.stringify(Object.fromEntries(healthByJob), null, 2),
  );
});
...
test('Grafana dashboard renders in the browser without console errors', async ({ page }) => {
  ...
  const relevantErrors = consoleErrors.filter((e) => !e.startsWith('[Plugins] Failed to preload plugin'));
  expect(
    relevantErrors,
