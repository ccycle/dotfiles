#!/usr/bin/env bash
# Orchestrates the Monitoring E2E suite: brings up the isolated stack
# (scripts/stack.sh), runs the Playwright spec against it, publishes the
# HTML+trace report, then tears the stack down. See
# tests/e2e-monitoring/design.md for the full rationale.
set -euo pipefail
...
cleanup() {
  ...
  if [ -d "$E2E_DIR/test-results/html" ] && mkdir -p "$REPORTS_DIR" 2>/dev/null; then
    ...
    echo "[e2e-monitoring] report published: https://reports.$(scutil --get LocalHostName 2>/dev/null || hostname -s).internal/${BRANCH_SLUG}/${SERVICE_NAME}/" >&2
  fi
  ...
  # worth preserving between runs - Grafana's local admin is created
  # fresh by its own GF_SECURITY_ADMIN_* env vars every start. See
  # design.md.
  "$STACK" teardown
}
trap cleanup EXIT
...
if [ ! -d node_modules ]; then
  echo "[e2e-monitoring] installing npm dependencies (first run)..." >&2
