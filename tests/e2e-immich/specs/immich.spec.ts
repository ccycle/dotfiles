...
async function loginAsAdmin(page: Page): Promise<void> {
  ...
  await page.waitForURL(/\/auth\/change-password|\/auth\/onboarding|\/photos|\/\?/, { timeout: 15_000 });
...
test('uploaded photo reflects into host filesystem and generates a thumbnail', async ({ page, request }) => {
  ...
  for (let attempt = 0; attempt < 40; attempt++) {
    ...
    const body = (await res.json()) as { assets: { items: Array<{ originalPath: string; thumbhash: string | null }> } };
  ...
  expect(thumbGenerated, 'thumbnail generated (thumbhash populated) within the poll window').toBe(true);

  expect(originalPath.startsWith(CONTAINER_UPLOAD_ROOT), `originalPath under ${CONTAINER_UPLOAD_ROOT}`).toBe(true);
