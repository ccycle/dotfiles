...
import { mkdirSync, mkdtempSync, rmSync, writeFileSync, existsSync, statSync, readdirSync } from 'node:fs';
...
function requireEnv(name: string): string {
  ...
  if (!v) throw new Error(`missing required env var ${name} - run via scripts/run.sh, not npx playwright test directly`);
  return v;
}
  ...
  try {
    ...
    await step('bootstrap a local admin user + API token (no OIDC/browser needed)', async () => {
      ...
      const createOut = fexec(
        '-u', 'git', 'forgejo', 'forgejo', 'admin', 'user', 'create',
        '--username', ADMIN_USER, '--password', password, '--email', ADMIN_EMAIL,
      ...
      adminToken = fexec(
        '-u', 'git', 'forgejo', 'forgejo', 'admin', 'user', 'generate-access-token',
        '-u', ADMIN_USER, '-t', 'e2e', '--scopes', 'all', '--raw',
      );
    ...
    await step('create a test repo via the Forgejo API', async () => {
      ...
      const getRes = await request.get(`/api/v1/repos/${ADMIN_USER}/${TEST_REPO}`, { headers: authHeaders() });
      expect(getRes.ok(), 'test repo exists after creation').toBeTruthy();
      note(`GET /api/v1/repos/${ADMIN_USER}/${TEST_REPO} -> ${getRes.status()} (confirmed it exists)`);
    });

    const repoDir = path.join(STATE_DIR, 'checkout');
    await step('push a minimal Forgejo Actions workflow', async () => {
      rmSync(repoDir, { recursive: true, force: true });
      const cloneUrl = BASE_URL.replace('http://', `http://${ADMIN_USER}:${adminToken}@`) + `/${ADMIN_USER}/${TEST_REPO}.git`;
      ...
      git('-C', repoDir, '-c', `user.email=${ADMIN_EMAIL}`, '-c', `user.name=${ADMIN_USER}`, 'add', '.forgejo/workflows/e2e.yaml');
      git('-C', repoDir, '-c', `user.email=${ADMIN_EMAIL}`, '-c', `user.name=${ADMIN_USER}`, 'commit', '-q', '-m', 'add e2e workflow');
    ...
    let secretFile = '';
    let configFile = '';
    await step('register a host-execution runner the way forgejo-runner-bootstrap does', async () => {
      mkdirSync(runnerDir, { recursive: true });
      secretFile = path.join(runnerDir, 'secret');
      configFile = path.join(runnerDir, 'config.yaml');

      const secret = fexec('-u', 'git', 'forgejo', 'forgejo', 'forgejo-cli', 'actions', 'generate-secret');
      writeFileSync(secretFile, secret, { mode: 0o600 });
      note('generated runner registration secret');

      const uuid = fexec(
        '-u', 'git', 'forgejo', 'forgejo', 'forgejo-cli', 'actions', 'register',
        '--secret', secret, '--name', RUNNER_NAME, '--labels', RUNNER_LABELS,
      );
      expect(uuid, 'runner UUID returned').not.toBe('');
      note(`registered runner "${RUNNER_NAME}" (labels: ${RUNNER_LABELS}), uuid=${uuid}`);

      const generatedConfig = execFileSync('nix', ['shell', 'nixpkgs#forgejo-runner', '-c', 'forgejo-runner', 'generate-config']).toString();
      ...
      execFileSync(
        ...
        { env: { ...process.env, RUNNER_URL: `${BASE_URL}/`, RUNNER_UUID: uuid, RUNNER_TOKEN_URL: `file:${secretFile}` } },
      );
      note('patched config.yaml with server URL/uuid/token and labels');

      runnerProc = spawn('nix', ['shell', 'nixpkgs#forgejo-runner', '-c', 'forgejo-runner', 'daemon', '--config', configFile], {
    ...
    let statusContext = '';
    await step('push, trigger the workflow on the runner, capture the real status context', async () => {
      ...
      for (let i = 0; i < 60; i++) {
        attempts = i + 1;
        const res = await request.get(`/api/v1/repos/${ADMIN_USER}/${TEST_REPO}/commits/${sha}/statuses`, { headers: authHeaders() });
        if (res.ok()) {
      ...
      note(`polled commit-status API (${attempts} attempt(s)) -> observed context: '${statusContext}'`);
      await attachLog('observed-status-context.txt', statusContext);
    });

    await step('apply branch protection via the API the way forgejo-branch-protection-bootstrap does', async () => {
      const createRes = await request.post(`/api/v1/repos/${ADMIN_USER}/${TEST_REPO}/branch_protections`, {
        headers: authHeaders(),
        data: {
          branch_name: 'main',
          enable_push: true,
      ...
      expect(createRes.ok(), `branch protection create: ${createRes.status()} ${await createRes.text()}`).toBeTruthy();
      note(`POST branch_protections {status_check_contexts: ["${statusContext}"]} -> ${createRes.status()}`);

      const getRes = await request.get(`/api/v1/repos/${ADMIN_USER}/${TEST_REPO}/branch_protections/main`, { headers: authHeaders() });
      expect(getRes.ok()).toBeTruthy();
      const protection = await getRes.json();
      expect(protection.enable_push, 'branch protection enable_push=true').toBe(true);
      expect(protection.enable_status_check, 'branch protection enable_status_check=true').toBe(true);
      expect(protection.status_check_contexts, 'branch protection status_check_contexts matches').toContain(statusContext);
      note('read back branch_protections/main and confirmed the fields stuck');
    });

    await step('force-push rejection (no separate "block force push" field exists - see design.md)', async () => {
      git('-C', repoDir, 'commit', '-q', '--allow-empty', '-m', 'rewrite for force-push test', '--amend');
      let rejected = false;
      try {
        execFileSync('git', ['-C', repoDir, 'push', '-q', '--force', 'origin', 'main'], { stdio: 'pipe' });
      } catch {
        rejected = true;
      }
    ...
    await step('forgejo dump lands on the host side of the bind mount', async () => {
      ...
      const dumpOut = fexec('-u', 'git', 'forgejo', 'forgejo', 'dump', '--file', containerDumpFile, '--type', 'zip');
      ...
      expect(existsSync(hostFile), 'forgejo dump produced a file visible on the host bind mount').toBe(true);
      expect(statSync(hostFile).size).toBeGreaterThan(0);
      note(`confirmed ${hostFile} exists on host (${statSync(hostFile).size} bytes)`);
    });

    await step(`retention pruning keeps exactly ${BACKUP_RETENTION_COUNT} generations`, async () => {
      ...
      try {
        ...
        note(`ran the retention one-liner -> ${remaining.length} generation(s) remain (expected ${BACKUP_RETENTION_COUNT})`);
      } finally {
