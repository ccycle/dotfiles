...
test('forgejo CI runner + branch protection + backup', async ({ request }, testInfo) => {
  ...
  try {
    let adminToken = '';
    await test.step('bootstrap a local admin user + API token (no OIDC/browser needed)', async () => {
    ...
    await test.step('create a test repo via the Forgejo API', async () => {
    ...
    await test.step('push a minimal Forgejo Actions workflow', async () => {
    ...
    let secretFile = '';
    let configFile = '';
    await test.step('register a host-execution runner the way forgejo-runner-bootstrap does', async () => {
    ...
    let statusContext = '';
    await test.step('push, trigger the workflow on the runner, capture the real status context', async () => {
    ...
    await test.step('apply branch protection via the API the way forgejo-branch-protection-bootstrap does', async () => {
    ...
    await test.step('force-push rejection (no separate "block force push" field exists - see design.md)', async () => {
    ...
    await test.step('forgejo dump lands on the host side of the bind mount', async () => {
    ...
    await test.step(`retention pruning keeps exactly ${BACKUP_RETENTION_COUNT} generations`, async () => {
