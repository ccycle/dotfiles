#!/usr/bin/env bash
# Orchestrates the Reports E2E suite: brings up the isolated stack
# (scripts/stack.sh), runs the Playwright spec against it, publishes the
# HTML+trace report, then tears the stack down. See
# tests/e2e-reports/design.md for the full rationale.
set -euo pipefail
...
cleanup() {
  ...
  if [ -d "$E2E_DIR/test-results/html" ] && mkdir -p "$REPORTS_DIR" 2>/dev/null; then
    ...
    echo "[e2e-reports] report published: https://reports.$(scutil --get LocalHostName 2>/dev/null || hostname -s).internal/${BRANCH_SLUG}/${SERVICE_NAME}/" >&2
  fi
...
if [ ! -d node_modules ]; then
  echo "[e2e-reports] installing npm dependencies (first run)..." >&2
